...
<p align="center">
  <!-- Replace with real screenshots/GIFs before submitting — see "Screenshots or a demo video" below. -->
  <em>Screenshots and a short demo video go here before submission.</em>
</p>

## Table of contents

- [How to set it up and run it locally](#how-to-set-it-up-and-run-it-locally)
  - [Prerequisites](#prerequisites)
  - [Environment variables](#environment-variables)
  - [Google Cloud / Gmail OAuth setup](#google-cloud--gmail-oauth-setup)
  - [Install and run](#install-and-run)
  - [Running tests](#running-tests)
  - [Optional — local AI provider via Ollama (dev-only, zero API cost)](#optional--local-ai-provider-via-ollama-dev-only-zero-api-cost)
- [Architecture decisions and trade-offs](#architecture-decisions-and-trade-offs)
  - [Architecture overview](#architecture-overview)
  - [Technology stack](#technology-stack)
  - [Key decisions and trade-offs](#key-decisions-and-trade-offs)
- [Screenshots or a demo video](#screenshots-or-a-demo-video)
...
- An Anthropic API key (or, for zero-cost local dev/testing, Ollama — see below)
...
| `AI_PROVIDER` | No | Dev-only. Set to `ollama` to route the assistant to a local Ollama model instead of Anthropic. **Ignored in production** — see below |
| `OLLAMA_BASE_URL` | No | Only used when `AI_PROVIDER=ollama`. Defaults to `http://localhost:11434` |
| `OLLAMA_MODEL` | No | Only used when `AI_PROVIDER=ollama`. No default — must name a model that supports tool/function calling |
...
4. **APIs & Services → Credentials → Create Credentials → OAuth client ID**:
   ...
   - Copy the generated **Client ID** and **Client Secret** into `.env.local` directly from the Console (avoid retyping — a single mistyped or truncated character produces a generic, hard-to-diagnose `401`/`invalid_client` error from Google with no indication of which field is wrong)
...
The assistant's Anthropic client is constructed behind one narrow interface (`AnthropicMessagesClient`, `src/lib/ai/client-types.ts`) that `runAssistant()` depends on but never inspects the concrete implementation of. This makes it possible to swap in a local model for development/testing without touching the tool-use loop, `validateAction()`, or dispatch — see [Key decisions and trade-offs](#key-decisions-and-trade-offs) for why this matters.

1. Install [Ollama](https://ollama.com/download).
2. Pull a model that actually supports tool/function calling — don't assume any given model does; `qwen2.5:7b` is confirmed working against this app's exact tool schemas (`qwen2.5:3b` was tried first and found unreliable for these scenarios; a 7B-class model is the practical floor here):
   ```
   ollama pull qwen2.5:7b
   ```
3. Start the server (`ollama serve`, or the desktop app) and confirm it's reachable at `http://localhost:11434`.
4. In `.env.local`, set:
   ```
   AI_PROVIDER=ollama
   OLLAMA_MODEL=qwen2.5:7b
   ```
5. Restart `npm run dev` so the new environment is loaded.

**Safety guarantees, not just defaults:** `selectProvider()` (`src/lib/ai/provider.ts`) forces Anthropic whenever `NODE_ENV=production`, regardless of what `AI_PROVIDER` is set to — a stray `AI_PROVIDER=ollama` left in a real deployment's environment can never silently swap the assistant onto an unauthenticated local model. `OLLAMA_MODEL` has no hardcoded default; a missing value fails loudly rather than guessing.

**Performance note:** on a memory-constrained machine, a ~5GB quantized model competing with the browser and dev server for RAM can take tens of seconds per turn, and Ollama unloads the model from memory after `OLLAMA_KEEP_ALIVE` (default 5 minutes) of inactivity, so many turns pay a cold-load cost. For a testing session, starting Ollama with a longer keep-alive avoids repeated reloads:
```
...
| AI assistant | `src/lib/ai/**` | Tool-use orchestration (Anthropic by default, optionally Ollama in dev), action validation, dispatch to the store |
...
- **@anthropic-ai/sdk** — Claude (`claude-opus-4-8`) with tool use for the assistant, the default and production provider
- **Ollama** (dev-only, optional) — local model provider for zero-cost assistant testing, behind the same tool-use seam
...
- **Human-in-the-loop send is enforced server-side, not just prompted.** The system prompt tells the model to always confirm before sending, but prompts aren't a security boundary. `validate-action.ts` rejects a `SEND_EMAIL` action unless a `REQUEST_SEND_CONFIRMATION` was issued for the *exact same payload* first — the confirmation is bound to a SHA-256 hash of the canonicalized email content (`src/lib/mail/canonical-payload.ts`), computed at the moment the user actually sees it and expiring after 10 minutes. A confirmation for one email can never authorize sending a different one, even if some confirmation happens to be pending, and a `SEND_EMAIL` that doesn't match is downgraded to a fresh confirmation request instead of silently dropped.
...
- **The AI provider sits behind one narrow interface, so Anthropic is never weakened to add a dev alternative.** `AnthropicMessagesClient` (`src/lib/ai/client-types.ts`) is the only thing `runAssistant()` depends on — a `{ messages: { create(params) } }` shape. The real Anthropic SDK client, the test suite's fakes, and the optional `OllamaMessagesClient` (`src/lib/ai/ollama-client.ts`) all satisfy it identically; none of the tool-use loop, validation, or dispatch code branches on which one it was given. `selectProvider()` (`src/lib/ai/provider.ts`) forces Anthropic whenever `NODE_ENV=production` regardless of any `AI_PROVIDER` setting — this was added specifically to test the assistant pipeline against a real local model at zero API cost, without risking any change to production behavior.
...
Claude (claude-opus-4-8) + tool definitions 1:1 with AppActionSchema
   — or, in dev with AI_PROVIDER=ollama, a local Ollama model behind
     the same AnthropicMessagesClient interface (never in production)
        │
...
Key files: `src/lib/ai/tools.ts` (tool definitions), `src/lib/ai/assistant.ts` (orchestration loop, max 4 turns), `src/lib/ai/validate-action.ts` (the security boundary — re-validates every action regardless of what the model claims), `src/lib/ai/dispatch-actions.ts` (client-side execution), `src/lib/ai/provider.ts` + `src/lib/ai/ollama-client.ts` (provider selection and the optional local adapter).

The assistant is never allowed to execute arbitrary code — it can only ever produce one of the ten action types in `AppActionSchema`, and every payload is schema-validated before dispatch, regardless of which provider produced it.
...
- `SEND_EMAIL` is only accepted when its payload's canonical hash matches a `REQUEST_SEND_CONFIRMATION` the client actually issued and shown to the user — the model cannot skip or spoof the confirmation step even if it tries to.
- The assistant cannot execute arbitrary code or arbitrary tool calls — its entire capability surface is the ten Claude tools in `tools.ts`, each mapped 1:1 to a validated action, regardless of which provider (Anthropic or the dev-only Ollama adapter) produced the tool call.
- The dev-only Ollama provider is forced off in production unconditionally (`selectProvider()` checks `NODE_ENV` first, before ever reading `AI_PROVIDER`) — it cannot be enabled by an environment misconfiguration in a real deployment.
...
- **The Ollama dev provider is CPU-inference-speed dependent.** On a memory-constrained machine it can take tens of seconds per turn, and is meant for local testing/cost-avoidance only — it is not a claim of production-grade assistant latency.
